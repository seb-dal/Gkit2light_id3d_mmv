...
Color ColorMaps::Terrain_Map(float height, float slope, float laplacien, float aireD, float wetness) {
	...
	if (height < 0.05) {
		...
		if (slope < 0.38)
			...
			return Gray * 0.5 + Gray * aireD * aireD * laplacien;
	}
	else if (height < 0.6) {
		if (slope < 0.32)
		...
		if (slope < 0.32)
			if (wetness > 0.3)
				return Color(0.15, 0.379, 0.0) * 0.5 + Gray * aireD * slope * laplacien;
			else
				return Color(0.465, 0.379, 0.0) * 0.5 + Gray * aireD * slope * laplacien;
		else
			return Gray * 0.5 + Gray * aireD * aireD * laplacien;
	}
	else {
